// Components
import Callout from '@source/components/interface/callout/Callout'
import Content from '@source/components/layout/content/Content'
import Heading from '@source/components/interface/heading/Heading'
import Link from '@source/components/interface/link/Link'
import Media from '@source/components/model/media/Media'

// Content: Works > Work > Body > 101 Extra ads - picker
function _101ExtraAdsPicker () {
  return (
    <>
      <Content>
        <Media
          width={768}
          type="image"
          className="media"
          path="/101-extra-ads-picker/teaser.png" />
        <p>101 Extra is a <strong>social game</strong>, based on traditional Turkish game Okey, developed and managed by Digitoy Games. It is available on Apple Store, Google Play Store, and Facebook.</p>
        <p>I designed advertisement visuals for 101 Extra <strong>in 2017</strong>. All images are used on Facebook as sponsored content.</p>
        <Callout
          size="large"
          intent="information"
          hasBackground={false}
          className="information">
          I also designed <Link to="/works/101-extra-facebook-page">101 Extra Facebook Page</Link>.
        </Callout>
        <Callout
          size="large"
          intent="information"
          hasBackground={false}
          className="information">
          Special thanks to <Link isExternal arrow="upright" to="https://cerenmurgan.com">Ceren</Link> for her significant contribution to this project!
        </Callout>
      </Content>
      <Content>
        <Heading
          rank={2}
          className="title">
          Facebook Ads
        </Heading>
        <Media
          width={768}
          type="image"
          className="media"
          path="/101-extra-ads-picker/01.png" />
        <Media
          width={768}
          type="image"
          className="media"
          path="/101-extra-ads-picker/02.png" />
        <Media
          width={768}
          type="image"
          className="media"
          path="/101-extra-ads-picker/03.png" />
        <Media
          width={768}
          type="image"
          className="media"
          path="/101-extra-ads-picker/04.png" />
        <Media
          width={768}
          type="image"
          className="media"
          path="/101-extra-ads-picker/05.png" />
      </Content>
    </>
  )
}

// Export
export default _101ExtraAdsPicker
